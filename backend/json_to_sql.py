...
for column_name in column_name_list:
    ...
    result_current_sql_list.append(sql)
...
for data in json_data:
    # result_current_sql_list.append("`, `".join(data.keys())+"`) VALUES (")
    for data_key in data.keys():
        result_current_sql_list.append("`"+data_key+"`, ")
    ...
    for data_key in data.keys():
        if(data_key == "image_list"):
            ...
            result_current_sql_list.append('`'+json_str+'`, ')
            continue
        result_current_sql_list.append(
            '`'+data[data_key].replace("\n", "\\n")+'`, ')
